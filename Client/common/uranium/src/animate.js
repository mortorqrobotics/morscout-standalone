function getPropValueGivenMediaQueries(styles, prop) {
  ...
  if (typeof styles === "number" || typeof styles === "string") return styles;
  ...
  for (const mq in styles) {
    ...
    if (global.matchMedia(mq.split("@media ")[1]).matches)
      finalValue = styles[mq][prop];
  }
  return finalValue;
}
...
function animatedStyle(prop, from, to, on) {
  ...
  if (prop === "transform") {
    ...
    const fromTransform = from.transform.reduce(
      (prev, curr) => ({ ...prev, ...curr }),
      {}
    );
    const toTransform = to.transform.reduce(
      (prev, curr) => ({ ...prev, ...curr }),
      {}
    );
    ...
    return {
      transform: Object.keys(animdTransforms).map(transform => ({
        [transform]: animdTransforms[transform]
      }))
    };
  }
