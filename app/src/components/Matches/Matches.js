  ...
  return (
    <View style={style.Matches.Table.teams}>
      <View style={style.Matches.Table.Cell}>
        <Link to={`/Scouting/${teams[0]}?Match=${mId}`}>
          {teams[0]}
        </Link>
      </View>
      <View style={style.Matches.Table.Cell}>
        <Link to={`/Scouting/${teams[1]}?Match=${mId}`}>
          {teams[1]}
        </Link>
      </View>
      <View style={Object.assign({}, style.Matches.Table.Cell, { borderRightWidth: 0 })}>
        <Link to={`/Scouting/${teams[2]}?Match=${mId}`}>
          {teams[2]}
        </Link>
      </View>
    </View>
...
export default @Uranium class Matches extends React.Component {
  static propTypes = {
    load: PropTypes.func.isRequired,
    // eslint-disable-next-line
    matches: PropTypes.object.isRequired, /* arrayOf(PropTypes.shape({
      ...
      <View style={style.Matches.main}>
        ...
        <View style={style.Matches.back}>
          <View css={style.Basic.Align.center}>
            <Title css={style.Basic.Title}>
                            All Matches
            </Title>
          </View>
          <View css={style.Basic.Align.center}>
            <InputFilter debounceTime={200} inputProps={{ style: style.Matches.Input.main }} />
          </View>
          <FilterResults
            items={state.matches}
                              ...
                              const matches = results.map((match) => {
                                ...
                                Object.entries(match).forEach((temp) => {
                                  ...
                                  if (
                                    ...
                                    m[data] = (
                                      <Link to={`/Match/${m.id}`} style={{ alignSelf: 'center' }}>
                                        {m[data]}
                                      </Link>
                                    );
                                  }
