...
export default (element, forceUpdate) => {
  ...
  if (!isWeb() || isWebVoidElement(element)) {
    const newStyle = Object.keys(css).reduce((styleAccumulator, property) => {
  ...
  const animatedValuesForMQ = Object.keys(css).reduce((animatedValues, key) => {
    ...
    const mqAnimatedValues = Object.keys(css[key]).reduce(
      (mqAnimatedValues, mqKey) => {
        if (
          typeof css[key][mqKey] !== "object" ||
          !css[key][mqKey].__getValue
        ) {
  ...
  const mqs = Object.keys(css).reduce((cssAccumulator, property) => {
    if (!property.match(/@media/)) return cssAccumulator;

    return `${cssAccumulator}${property}{.${URANIUM_CLASSNAME}.${className}{${createCSSDeclarations(
      expandStyle(css[property])
    )}}}`;
  }, "");
  ...
  const styleTag = props.children.filter(
    child => child && child.type === "style"
  )[0];
