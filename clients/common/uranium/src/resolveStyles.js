...
const _resolveProps = (element, forceUpdate) => {
  ...
  return React.cloneElement(element, omit(newProps, "key", "ref"));
};

const _runPlugins = (element, forceUpdate) => {
  if (!React.isValidElement(element) || !element.props.css) {
    return element;
  }
...
resolveStyles = (element, forceUpdate) =>
  [_resolveChildren, _resolveProps, _runPlugins].reduce(
